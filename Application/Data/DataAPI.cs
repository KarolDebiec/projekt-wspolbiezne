...
namespace Data
{
    public abstract class DataAbstractAPI : IObserver<int>, IObservable<int>
    {
        ...
        private class DataApi : DataAbstractAPI
        {
            ...
            {
                return this.ballRepository.GetBall(ballId).posX;
            }

            public override double getBallPositionY(int ballId)
            {
                return this.ballRepository.GetBall(ballId).posY;
            }
            ...
            {
                return this.ballRepository.GetBall(ballId).offsetX;
            }

            public override double getBallSpeedY(int ballId)
            {
                return this.ballRepository.GetBall(ballId).offsetY;
            }

            public override void setBallSpeed(int ballId, double speedX, double speedY)
            {
                this.ballRepository.GetBall(ballId).offsetX = speedX;
                this.ballRepository.GetBall(ballId).offsetY = speedY;
            }
            ...
            {
                barrier = new Barrier(amount);
                ballRepository.CreateBalls(amount);
